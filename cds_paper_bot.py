...
def process_images(
    ...
    for image_file in downloaded_image_list:
        ...
        if img_size > MAX_IMG_SIZE:
            ...
            continue
        # convert is not available by default
    ...
    if post_gif:
        ...
        while img_size > MAX_IMG_SIZE:
            ...
            if img_size > MAX_IMG_SIZE:
                ...
                logger.info(
                    f"... image too big ({img_size} bytes), dropping last figure, {len(images_for_gif)} images in GIF"
                )
...
def load_config(experiment, feed_file, auth_file):
    ...
    if experiment not in config.sections():
        logger.error(f"Experiment {experiment} not found in {auth_file}")
    config_dict["AUTH"] = {}
    for key in config[experiment]:
